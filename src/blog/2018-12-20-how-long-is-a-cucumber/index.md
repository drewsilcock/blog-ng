...
image: charles-deluvio-gtuwf3agci0-unsplash.jpg
unsplash-credit:
  username: charlesdeluvio
  full-name: Charles Deluvio
tags:
  - Coding
keywords:
  - unicode
...
The astute reader may wonder what these surrogate pair representations of single characters means for the indexing of strings in JavaScript. When you have a string like `js›var s = "hello there"`, you expect `js›s[0]` to give you the first character, `js›s[3]` to give you the fourth character and `js›s[7]` to give you the eight character. But what about the following code:
...
This means that the string indexing works the same way as classical C-like array indexing, where `c›s[4]` just means getting the address of `s` and skipping forward `c›4 * sizeof s[0]` bytes.
...
- **C** - The standard types used are `c›char` which is generally used as an 8-bit character for ASCII (and sometimes for other purposes where `c›uint8_t` should *really* be used instead) and `c›wchar_t` (introduced in C90) for handling any Unicode code point. In truth, the standard does not specify the size of either `c›char` or `c›wchar_t` [See note 3 for more information about this](#note-4).
- **C++** - natively uses 8-bit `cpp›std::string` much like pure C. There is `cpp›std::wstring` analogous to C's `c›wchar_t` with corresponding `cpp›std::wcout`, `cpp›std::wcerr`, etc.
...
- **Java** - Java's `java›char` type is 16-bit length able to store the BMP characters only. The `java›String` type uses UTF-16 to enable the full Unicode range as per JavaScript.
...
- **D** - has standard library support for UTF-8, UTF-16 *and* UTF-32 via `d›string`, `d›wstring` and `d›dstring` respectively, so you're spoilt for choice!
- **Rust** - uses UTF-8 strings as standard - Rust source code is UTF-8, string literals are UTF-8, the `rust›std::string::String` encapsulates a UTF-8 string and primitive type `rust›str` (the borrowed counterpart to `rust›std::string::String`) is always valid UTF-8. Nice!
...
- `rust›my_string.bytes()` for iterating over raw bytes - each item is given as a `u8`.
- `rust›my_string.chars()` for iterating over code points (technically it is for iterating over [Unicode Scalar Values](http://www.unicode.org/glossary/#unicode_scalar_value) which is basically a Unicode code point excluding the low and high surrogates discussed earlier) - each item is given as a `rust›char`, which is a 32-bit long representation of a single codepoint.
...
C90 defines `c›wchar_t` as "an integral type whose range of values can represent distinct codes for all members of the largest extended character set specified among the supported locales" (ISO 9899:1990 §4.1.5).

This is a classic example of C specifications not really giving away enough details to really nail down an API, which is one of the reasons that undefined behaviour is *so easy* to accidentally stumble upon in C. Classic examples of under-defined standards in C is that `c›char` need not be 8 bits long, `c›int` not needing to be 16 bits long, `c›double` not needing to be 4 bytes - all of these are left up to the compiler to implement.
